...
const CTA_LABELS: Partial<Record<Lang, { cta: string; whatsapp: string }>> = {
  en: { cta: "Check availability", whatsapp: "WhatsApp" },
  tr: { cta: "Müsaitliği kontrol et", whatsapp: "WhatsApp" },
  ar: { cta: "تحقق من التوفر", whatsapp: "واتساب" },
  ru: { cta: "Проверить наличие", whatsapp: "WhatsApp" },
...
export default function StickyCtaBar({ phone, lang }: Props) {
  ...
  const handleWhatsApp = () => {
    if (typeof window !== "undefined" && (window as unknown as Record<string, unknown>).posthog) {
      (window as unknown as { posthog: { capture: (e: string) => void } }).posthog.capture("whatsapp_click");
    }
  };
  ...
  return (
    ...
    >
      <Link
        ...
        onClick={() => {
          if (typeof window !== "undefined" && (window as unknown as Record<string, unknown>).posthog) {
            (window as unknown as { posthog: { capture: (e: string, p: Record<string, string>) => void } }).posthog.capture("cta_click", { label: "check_availability", destination: "contact" });
          }
        }}
      >
        {labels.cta}
